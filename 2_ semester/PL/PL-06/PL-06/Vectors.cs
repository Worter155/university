    ...
    public static class Vectors
    {
        ...
        public static IVectorable InputVector(Stream st)
        {
            ...
            else
            {
                ...
                IVectorable vector = new ArrayVector();
                while (st.Position != st.Length)
                {
                    byte[] leng = new byte[4];
                    st.Read(leng, 0, 4);
                    vector = new ArrayVector(BitConverter.ToInt32(leng, 0));
        ...
        {
            IVectorable vector = new ArrayVector();
            string[] res = st.ReadLine().Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            vector = new ArrayVector(res.Length-1);
